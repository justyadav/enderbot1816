...
# =========================================================
# INTERNAL SECURITY UTILITY LAYERS
# =========================================================
async def verify_guild_access(guild_id: int) -> bool:
    """Ensures the authenticated browser session possesses administrative access to the targeted cluster."""
    ...
    for g in all_guilds:
        if int(g["id"]) == guild_id:
            # Check for the strict ADMINISTRATOR permission bitmask (0x8)
...
@app.route("/login")
async def login():
    # Hardens the initial handshake using a secure anti-CSRF state tracking token
    state_token = secrets.token_urlsafe(16)
    session["oauth_state"] = state_token
    oauth_url = f"https://discord.com/api/oauth2/authorize?client_id={CLIENT_ID}&redirect_uri={REDIRECT_URI}&response_type=code&scope=identify%20guilds&state={state_token}"
...
async def callback():
    ...
    # Verify transaction authenticity via anti-CSRF matching state token checks
    if not state or state != session.get("oauth_state"):
        return "Security tracking token state mismatch. Access denied.", 403
    ...
    async with aiohttp.ClientSession() as session_client:
        async with session_client.post("https://discord.com/api/oauth2/token", data=data, headers=headers) as resp:
            ...
            session.pop("oauth_state", None) # Clean up security state token
...
async def manage_server(guild_id):
    ...
    # Structural Protection Check: Prevent unauthorized users from modifying target configuration clusters
    if not await verify_guild_access(guild_id):
        return "Access denied: Missing necessary backend administrative permissions.", 403
...
async def update_server_settings(guild_id):
    ...
    # Structural Protection Check: Prevent post-request spoofing/manipulation vectors
    if not await verify_guild_access(guild_id):
        return "Access denied: Missing necessary backend administrative privileges.", 403
